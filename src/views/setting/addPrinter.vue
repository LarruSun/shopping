...
<div class="contain">
	...
	<div class="common-title">
	  <div>
	    ...
	    <span class="active-title">新增打印机</span>
	  </div>
	</div>
	<div class="form-contain">
	<el-form :model="formData" :rules="rules" ref="ruleForm" label-position="left" label-width="120px">
		<el-form-item label="厂商 :">飞鹅打印机</el-form-item>
		<el-form-item label="编号(SN) :" prop="sn">
			<el-input placeholder="请输入SN号" v-model="formData.sn"></el-input>
		</el-form-item>
		<el-form-item label="密钥(KEY) :" prop="key">
			<el-input placeholder="请输入密钥" v-model="formData.key"></el-input>
		</el-form-item>
		<el-form-item label="打印机名称 :" prop="printName">
			<el-input placeholder="请输入打印机名称" v-model="formData.printName"></el-input>
		</el-form-item>
	...
	data(){
		return {
			...
			rules:{//表单验证规则
		          ...
		          key: [
		            { type: 'string',required: true, message: '请输入密钥', trigger: 'blur' }
		          ],
		          printName: [
		            { type: 'string',required: true, message: '请输入打印机名称', trigger: 'blur' }
		          ],
		          printType: [
		            { type: 'string', message: '请选择打印类型', trigger: 'change' },
		          ],
		          printNum: [
		            { type: 'string', message: '请选择打印份数', trigger: 'change' }
		          ]
		      }
			}
		},
